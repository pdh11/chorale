    ...
    : Recordset(parent)
{
    ...
    Connection::infomap_t::const_iterator ci = parent->m_infomap.find(id);
...
CollateRecordset::CollateRecordset(Connection *db, unsigned int field)
    : m_parent(db),
      m_field(field),
      m_start(0),
      m_index(0),
...
SearchRecordset::SearchRecordset(Connection *db, const std::string& query)
    : Recordset(db),
      m_query(query),
      m_start(0),
      m_index(0),
